...
jVec3.prototype.Transform = function(matrix, noTranslate = false)
{
    var x = matrix.m[0][0] * this.x + matrix.m[0][1] * this.y + matrix.m[0][2] * this.z + (noTranslate ? 0.0 : matrix.m[0][3]);
    var y = matrix.m[1][0] * this.x + matrix.m[1][1] * this.y + matrix.m[1][2] * this.z + (noTranslate ? 0.0 : matrix.m[1][3]);
    var z = matrix.m[2][0] * this.x + matrix.m[2][1] * this.y + matrix.m[2][2] * this.z + (noTranslate ? 0.0 : matrix.m[2][3]);
    var w = matrix.m[3][0] * this.x + matrix.m[3][1] * this.y + matrix.m[3][2] * this.z + (noTranslate ? 0.0 : matrix.m[3][3]);
...
jVec3.prototype.IsNearlyEqual = function(value)
{
    return (IsNearlyZero(this.x - value.x) && IsNearlyZero(this.y - value.y) && IsNearlyZero(this.z - value.z));
}

jVec3.prototype.IsGreaterThan = function(value)
{
    if (this.x - value.x > epsilon)
        return true;
    
    if (this.y - value.y > epsilon)
        return true;
    
    return false;
}

jVec3.prototype.MakeHashKey = function()
{
...
var jPlane = function()
{
    this.n = CreateVec3(0.0, 0.0, 0.0, 0.0);
    this.d = 0.0;
}

var CreatePlane = function(a, b, c, d)
{
    var newPlane = new jPlane();
    newPlane.n.x = a;
    newPlane.n.y = b;
    newPlane.n.z = c;
    newPlane.d = d;
...
var CreatePerspectiveMatrixFarAtInfinity = function(width, height, fovRadian, near)
{
    ...
    projMat.m[0][0] = F*(height/width); projMat.m[0][1] = 0.0;      projMat.m[0][2] = 0.0;                      projMat.m[0][3] = 0.0;
    projMat.m[1][0] = 0.0;              projMat.m[1][1] = F;        projMat.m[1][2] = 0.0;                      projMat.m[1][3] = 0.0;
    projMat.m[2][0] = 0.0;              projMat.m[2][1] = 0.0;      projMat.m[2][2] = -1.0;                     projMat.m[2][3] = -(2.0*near);
    projMat.m[3][0] = 0.0;              projMat.m[3][1] = 0.0;      projMat.m[3][2] = -1.0;                     projMat.m[3][3] = 0.0;
    return projMat;
}
...
var IsNearlyZero = function(value)
{
    return (Math.abs(value) < epsilon)
}

var IsNearlyEqual = function(a, b)
{
